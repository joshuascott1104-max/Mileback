...
import { Car, Receipt, Download, ArrowRight, Plus, CheckCircle2 } from 'lucide-react'
...
function QuickExpense({ onSaved }) {
  ...
  const [open, setOpen] = useState(false)
  const [saved, setSaved] = useState(false)
  const [form, setForm] = useState({ amount: '', category: 'Parking', description: '' })
  ...
  const handleSave = () => {
    ...
    setSaved(true)
    setForm({ amount: '', category: 'Parking', description: '' })
    onSaved?.()
    setTimeout(() => { setSaved(false); setOpen(false) }, 1800)
  }
